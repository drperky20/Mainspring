...
`POST /chat/stream` is an additive operator-authorized transport for console chat. It authenticates before parsing the bounded body, starts the same RunLog-first ingress, and emits AI SDK v1 start, run-metadata, text, error, and finish chunks without exposing private RunLog payloads. Compatibility-only gateways use the existing public event facade behind the same transport.
