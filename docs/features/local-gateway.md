...
`POST /chat/stream` is an additive operator-authorized transport for console chat. It authenticates before parsing the bounded body and starts the same RunLog-first ingress. `GatewayChatStream` owns the polling, public-event projection, deduplication, terminal closure, and AI SDK v1 start/run-metadata/text/error/finish chunks without exposing private RunLog payloads. Compatibility-only gateways use the existing public event facade behind the same transport.
...
`GatewayRunControl` owns run ingress, cancellation, and approval responses. It persists hash-only `run.enqueue` authority before either the compatibility mailbox enqueue or a RunLog `RunIntent` reaches its runtime store, `run.cancel` before mailbox or RunLog cancellation, and `approval.resolve` before a legacy approval response or RunLog receipt can mutate lifecycle state. Run/session and approval/run/session bindings fail closed before authority is written. Outcome and failure rows retain the decision link without storing prompts, cancellation reasons, approval reasons, or response contents; hosted routes use the authenticated principal instead of browser attribution.
