...
| Cron/headless side-effect escalation | Denied by default and records a cron `DecisionRecord`; RunLog-configured gateway cron schedules use the same fail-closed policy path. | `src/security/agent-security-regression.test.ts`; `RunLogCron`; `SqliteRunLogStore`; `src/gateway/LocalGateway.test.ts`. | Gateways without a RunLog host remain mailbox-compatible; operator-facing cron grant review/create UX is pending. |
