...
| Skill memory poisoning | Memory and skill writes are scanned and staged on review findings, with local operator review/apply surfaces. | `src/provenance/ProvenanceReview.test.ts`; `src/gateway/LocalGateway.test.ts`; `src/gateway/server/createLocalGatewayServer.test.ts`; `MemoryTool`; `SkillTools`. | Taint labels beyond scan findings remain pending. |
