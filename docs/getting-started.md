...
`provider-run` uses the canonical RunLog SDK host and does not require a live provider key. `tool-approval` uses the same host to show an approval-gated workspace write. `coding-agent` uses the same host for a read-then-approved-write coding workflow. `personal-assistant` and `support-agent` use the same host for read-only workspace tool runs. `agency-client-agent` uses the same host for a read-then-approved-write client deliverable flow. `local-first-agent` keeps memory compatibility coverage while migration continues. The examples do not provide containment; host tool examples inherit the normal local runtime limits.
