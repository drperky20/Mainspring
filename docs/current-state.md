...
- Non-tool host surfaces such as channel sends, provider config mutation, artifact publish, full visual cron grant controls, and future subagent creation still need explicit `DecisionRecord` adapters as those surfaces become RunLog-native.
...
- Dedicated in-app visual controls for creating and reviewing scoped cron grants; the gateway API/client surface exists.
