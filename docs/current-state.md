...
- Non-tool authority decisions use the same durable `DecisionRecord` shape:
  ...
  - `GatewayRunControl` owns gateway run ingress and approval responses. It persists hash-only `run.enqueue` authority before compatibility or RunLog enqueue, and `approval.resolve` authority before legacy runtime resolution or RunLog receipt creation; outcome/failure rows retain the decision link, and hosted routes supply the authenticated principal instead of browser attribution.
