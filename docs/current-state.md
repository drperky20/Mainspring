...
- Explicit RunLog gateway/API lane:
  ...
  - `/runlog/runs/start` creates `RunIntent` records and returns sanitized `RunLogProjection` responses. `POST /runlog/runs/:runId/retry` accepts only failed or cancelled canonical gateway work, records operator authority first, and creates a distinct child run linked by `parentRunId`; it does not rewind checkpoints or replay stored tool results. Headless scheduler work remains restricted to the cron grant path.
...
- Live SaaS-style console:
  ...
  - `ConsoleNavigation.tsx` owns primary/activity navigation; `ConsoleLocation.ts` owns the validated shareable screen/activity/run URL state; `ConsoleProviderCatalog.tsx`, `ConsoleSetup.tsx`, `ConsoleClientForms.tsx`, `ConsoleSettings.tsx`/`ConsoleSettings.css`, `ConsoleClientWorkspaceFrame.tsx`, and `ConsoleClientWorkspaceViews.tsx` own provider, first-run, client-editing, settings, selected-client workspace-frame, and client feature-view UI/style boundaries; `controlRoom.css` owns shell, activity, state, and responsive control-room styling while `controlRoomWorkspace.css` owns client/setup/chat/automation feature styling; `ConsoleWorkflowPrimitives.tsx` owns shared dialogs, including labelled initial focus, Escape dismissal, Tab cycling within the dialog, and focus return, plus the action rail, tool toggles, and automation-flow rendering; `useConsoleRunActivity.ts` owns scoped run streaming; `useCompatibilityRunEvents.ts` owns session-bound abortable compatibility trace reads; and dedicated cursor hooks load RunLog, canonical tool-call, compatibility, approval, trace, usage, artifact, audit, and per-workspace memory pages only while their Activity surface is visible.
  ...
  - The run detail view exposes the sanitized event timeline, tool calls, checkpoints, policy decisions, errors, native RunLog or compatibility-run cancellation, and a fresh-run retry action for failed/cancelled canonical work. Selected RunLog details can be restored from a validated URL deep link without putting session paths, tokens, prompts, provider material, or host paths in the address bar; both canonical trace paging and compatibility event reads discard stale responses when the operator changes runs.
...
- Non-tool authority decisions use the same durable `DecisionRecord` shape:
  ...
  - `GatewayCronControl` records hash-only `cron.schedule.write` decisions before schedule create/update/delete, `cron.grant.create` before a scoped grant can change schedule metadata, and `cron.trigger` before manual or due-trigger schedule cursor mutation and dispatch. Hosted gateway routes supply the authenticated principal as the actor, overriding request-body attribution; the trigger decision ID is carried into RunLog metadata.
  ...
  - `GatewayRunControl` owns gateway run ingress, cancellation, fresh retry, and approval responses. It persists hash-only `run.enqueue` authority before compatibility or RunLog enqueue, `run.cancel` before mailbox or RunLog cancellation, `run.retry` before a linked canonical child is queued, and `approval.resolve` before legacy runtime resolution or RunLog receipt creation; outcome/failure rows retain the decision link, run/session binding is checked fail-closed, and hosted routes supply the authenticated principal instead of browser attribution.
  - `GatewayRunInputResolver` owns the shared host-side binding boundary for gateway and RunLog ingress plus cron preparation: session metadata, workspace/client state, agent ownership, provider-profile compatibility, archived-resource checks, managed credential references, and registered runtime profiles are resolved before a run input is handed to a runtime.
  - artifact handling in the gateway is read-only indexing of runtime-created artifacts; the gateway has no artifact-publish mutation surface. `GatewayArtifactProjection` derives compatibility inventory paths only from bounded runtime artifact IDs, while `GatewayArtifactAccess` realpath-checks the persisted row against the runtime artifact root and opens the file before the HTTP server streams it. Symlink/root escapes fail closed and invalid media types fall back to `application/octet-stream`.
  - `GatewayProjectionSynchronizer` owns the restart-safe catch-up pass that reads native mailbox and RunLog facts through callbacks, then delegates approval, usage, tool-call, artifact, execution-cell, and terminal-lease materialization to projection-only collaborators. `GatewayUsageProjection` owns native and RunLog usage normalization, pricing, idempotency, and budget-transition callbacks. Artifact and usage collections are read-only through `LocalGatewayAppStateStore`; authoritative runtime materialization uses its explicit `projections` namespace, and malformed negative/non-finite ledger values cannot reduce operator totals. Broad snapshots retain only a bounded newest-first detail tail while indexed history counts and complete provider/model usage rollups preserve operator totals; Activity routes remain the full cursor-paginated read surface.
...
- Local-agent security regression coverage now exists in `src/security/agent-security-regression.test.ts` with a companion matrix in `docs/security-redteam-matrix.md`.
  ...
  - The optional `PlaywrightBrowserRuntimeAdapter` provides a host-owned, run-scoped browser lease with bounded refs, direct public-target revalidation after page actions, sanitized browser URLs, screenshot/trace artifact events, and release hooks. Unsafe redirects close the lease, and the RunLog session factory closes it on execution abort; the matrix records that browser automation is not an isolation boundary and keeps hosted browser identity and browser persistence outside this package.
...
- Console chat dispatch uses the authenticated `POST /chat/stream` adapter. `GatewayChatStream` owns its bounded public-event-to-AI-SDK projection and terminal stream lifecycle, while the console lazy-loads the AI transport only when chat is submitted; automation and compatibility callers retain their existing dispatch contracts.
...
- Arbitrary checkpoint rewind and replay remain intentionally unimplemented. Operator retry is limited to a fresh linked run for failed/cancelled canonical gateway work; prior checkpoint events and stored tool results are never copied into the child run.
...
GitHub CI runs the same `pnpm verify` contract on Linux, macOS, and Windows for every push and pull request, plus a real Chromium `pnpm console:e2e` job whose gateway/Vite/Playwright logs stream to the job output and whose trace, screenshot, video, and HTML evidence is retained on failure. The Windows verify leg preflights `Microsoft.PowerShell.Security` and selects `pwsh` for managed-secret integration. The release-check workflow invokes the canonical `pnpm release:check` script instead of maintaining a second hand-copied command list; the repository workflow verifier enforces pnpm pinning, frozen installs, timeouts, permissions, concurrency, browser coverage, and release parity.
