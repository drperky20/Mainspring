...
- Live SaaS-style console:
  ...
  - `ConsoleNavigation.tsx` owns primary/activity navigation; `ConsoleProviderCatalog.tsx`, `ConsoleSetup.tsx`, `ConsoleClientForms.tsx`, `ConsoleSettings.tsx`/`ConsoleSettings.css`, and `ConsoleClientWorkspaceFrame.tsx` own provider, first-run, client-editing, settings, and selected-client workspace-frame UI/style boundaries; `ConsoleWorkflowPrimitives.tsx` owns shared dialogs, including labelled initial focus, Escape dismissal, Tab cycling within the dialog, and focus return, plus the action rail, tool toggles, and automation-flow rendering; `useConsoleRunActivity.ts` owns scoped run streaming; and dedicated cursor hooks load RunLog, canonical tool-call, compatibility, approval, trace, usage, artifact, audit, and per-workspace memory pages only while their Activity surface is visible.
