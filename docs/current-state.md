...
- Live SaaS-style console:
  ...
  - The run detail view exposes the sanitized event timeline, tool calls, checkpoints, policy decisions, errors, and native RunLog or compatibility-run cancellation. Selected RunLog details can be restored from a validated URL deep link without putting session paths, tokens, prompts, provider material, or host paths in the address bar; both canonical trace paging and compatibility event reads discard stale responses when the operator changes runs.
...
- Non-tool authority decisions use the same durable `DecisionRecord` shape:
  ...
  - `GatewayRunControl` owns gateway run ingress, cancellation, and approval responses. It persists hash-only `run.enqueue` authority before compatibility or RunLog enqueue, `run.cancel` before mailbox or RunLog cancellation, and `approval.resolve` before legacy runtime resolution or RunLog receipt creation; outcome/failure rows retain the decision link, run/session binding is checked fail-closed, and hosted routes supply the authenticated principal instead of browser attribution.
