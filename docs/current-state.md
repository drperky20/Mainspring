...
- Gateway snapshot transport:
  ...
  - `GET /usage-history` delegates to `UsageHistoryPage`, requires gateway app-state metadata, and returns reverse-paginated browser-safe ledger rows only while the Activity Usage view is open. The aggregate snapshot still supplies compatibility summaries and provider/model breakdowns; artifacts, audit, and memory do not yet have their own detailed history pages.
- Live SaaS-style console:
  ...
  - The first-level UI is organized around Home, Workspaces, Activity, and Settings. Runs, approvals, and usage remain explicit Activity views; Usage loads its detailed ledger incrementally while its view is open; loading, stale, offline, unauthorized, empty, and error states remain visible.
