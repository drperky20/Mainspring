...
- Non-tool authority decisions use the same durable `DecisionRecord` shape:
  ...
  - `GatewayUsageProjection` owns native and RunLog usage normalization, pricing, idempotency, and budget-transition callbacks. Artifact and usage collections are read-only through `LocalGatewayAppStateStore`; authoritative runtime materialization uses its explicit `projections` namespace, and malformed negative/non-finite ledger values cannot reduce operator totals.
