...
| RunLog crash recovery | RunLog has SQLite WAL events, leases, checkpoints, and scoped approval resume for implemented tool boundaries. It does not yet continue the provider loop after the approved tool result. | "RunLog checkpoints are implemented", "scoped approval resume exists for approved tool boundaries" | "fully resumable approval workflow" | `src/core`, `src/adapters/sqlite`, `docs/current-state.md` |
| Approval receipts | Legacy receipts bind approval id/run/tool/input/expiry/nonce. RunLog receipts add HMAC signing, private request snapshots, expiry, and one-time-use rows for approved tool resume. | "scoped signed RunLog receipts", "one-time-use approved tool resume" | "broad approval token", "secure containment", "operator role system" | `src/policy/ApprovalReceipt.ts`, `src/core/RunLogApprovalReceipt.ts`, `src/adapters/sqlite/SqliteRunLogStore.ts` |
| Cron/headless | RunLog cron rows enqueue normal runs; stricter scoped grants are not complete. | "cron creates ordinary RunLog runs", "headless policy hardening planned" | "cron can safely run any approved automation unattended" | `src/adapters/sqlite/SqliteRunLogStore.ts`, `docs/runtime-loop.md` |
