...
| Cron/headless | RunLog cron rows and RunLog-configured gateway cron schedules emit policy decisions before queue/fail, deny side-effecting headless schedules by default, and support scoped grants bound to prompt hash, schedule hash, allowed tools, expiry, and execution count. Gateway HTTP/client grant preview/create surfaces exist; dedicated visual console controls remain pending. Gateways without a RunLog host keep mailbox compatibility. | "RunLog cron creates ordinary audited runs", "side-effecting headless RunLog cron requires scoped grants", "RunLog-configured gateway cron uses the same policy path", "cron grant API/client surfaces exist" | "cron can safely run any approved automation unattended", "every gateway cron surface is fully migrated", "cron grants have complete in-app visual controls" | `src/capabilities/cron/RunLogCron.ts`, `src/adapters/sqlite/SqliteRunLogStore.ts`, `src/core/RunLogKernel.test.ts`, `src/gateway/LocalGateway.test.ts`, `src/gateway/server/createLocalGatewayServer.test.ts`, `apps/console/src/localGatewayClient.test.ts`, `docs/runtime-loop.md` |
| Memory/skills | Memory and skill writes can alter future behavior. Runtime memory and skill writes now scan and can stage review records, but operator-facing review UX and remote trust are incomplete. | "memory/skill writes are behavior mutation", "local provenance scan exists", "remote trust not implemented" | "safe self-improving skill marketplace" | `src/provenance`, `src/memory`, `src/skills`, `docs/skills-security.md` |
