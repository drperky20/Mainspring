...
| RunLog crash recovery | RunLog has SQLite WAL events, leases, and checkpoints for implemented paths. Full approval resume is not implemented yet. | "RunLog checkpoints are implemented", "approval resume is planned" | "fully resumable approval workflow" | `src/core`, `src/adapters/sqlite`, `docs/current-state.md` |
| Approval receipts | Legacy receipts bind approval id/run/tool/input/expiry/nonce. RunLog crypto resume is not complete. | "approval-aware", "receipts exist for legacy path" | "durable cryptographic approval resume" | `src/policy/ApprovalReceipt.ts`, `docs/current-state.md` |
