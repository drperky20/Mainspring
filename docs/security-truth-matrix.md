...
| Cron/headless | RunLog cron rows and RunLog-configured gateway cron schedules emit policy decisions before queue/fail, deny side-effecting headless schedules by default, and support scoped grants bound to prompt hash, schedule hash, allowed tools, expiry, and execution count. Gateways without a RunLog host keep mailbox compatibility, and operator-facing grant creation/review UX is still pending. | "RunLog cron creates ordinary audited runs", "side-effecting headless RunLog cron requires scoped grants", "RunLog-configured gateway cron uses the same policy path" | "cron can safely run any approved automation unattended", "every gateway cron surface is fully migrated", "cron grants have a complete operator UX" | `src/capabilities/cron/RunLogCron.ts`, `src/adapters/sqlite/SqliteRunLogStore.ts`, `src/core/RunLogKernel.test.ts`, `src/gateway/LocalGateway.test.ts`, `docs/runtime-loop.md` |
