...
`examples:check` is an alias for the runnable examples smoke suite. It includes the RunLog-native `provider-run`, `tool-approval`, `personal-assistant`, `support-agent`, and `agency-client-agent` examples plus compatibility examples that still exercise coding and memory flows during migration.
