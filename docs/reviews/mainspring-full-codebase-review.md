...
| Run ingress and approval responses | `GatewayRunControl` records hash-only `run.enqueue` authority before compatibility or RunLog enqueue and `approval.resolve` before legacy or RunLog approval lifecycle mutation; outcome/failure rows retain the decision link. Hosted requests bind the actor to the authenticated principal. | Prompt, approval reason, and response contents remain hash-only; browser request-body actors are ignored by the route schemas. | Gateways without app-state remain compatibility-only and have no durable gateway audit store. |
