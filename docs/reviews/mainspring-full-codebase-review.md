...
| Bounded history is uneven | Activity, approvals, public traces, and detailed usage are paginated; the primary snapshot still aggregates numerous unrelated collections. | Long-lived local state can increase snapshot cost. | Preserve compatibility now; add endpoint/read-model pagination for artifacts, audit, and memory as concrete callers migrate. |
...
| Inspect outcomes | Runs, artifacts, tool rows, checkpoints, usage, and trace panels exist. | Detailed usage now loads through a bounded page; artifact/audit/memory histories still arrive through the aggregate. | Public RunLog trace and usage pages are bounded; progressively disclose the remaining read models. |
