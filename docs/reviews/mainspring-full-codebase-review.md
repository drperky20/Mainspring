...
- `LocalGateway` is the public facade for clients, workspaces, agents,
  ...
  to `GatewayTopologyControl`, rather than being implemented in the facade.
...
| Gateway administration | Hosted auth/session attribution and role authorization before mutations; provenance review, provider profiles, cron grant issuance, budget control, deployment control, and topology control ignore browser-supplied attribution when a hosted principal exists. Topology updates also reject cross-client workspace binding. | User/password verifier material, decision metadata, and workspace-root evidence are omitted from DTOs/audit inputs. | Local roles are not tenancy, SSO, or enterprise identity. |
