...
| Cron | `GatewayCronControl` records hash-only schedule/grant authority before app-state mutation and `cron.trigger` before manual/due cursor mutation or dispatch; scoped, expiring grants and RunLog decisions govern side-effecting work. Hosted requests bind audit actor to the authenticated principal. | Grant projections and browser audit DTOs omit private hashes and decision metadata; trigger IDs are internal RunLog metadata. | Compatibility gateway cron remains mailbox-backed without a RunLog host. |
...
| Run ingress, cancellation, and approval responses | `GatewayRunControl` records hash-only `run.enqueue`, `run.cancel`, and `approval.resolve` authority before compatibility or RunLog lifecycle mutation; run/session and approval/run/session bindings fail closed; outcome/failure rows retain the decision link. Hosted requests bind the actor to the authenticated principal. | Prompt, cancellation reason, approval reason, and response contents remain hash-only; browser request-body actors are ignored by the route schemas. | Gateways without app-state remain compatibility-only and have no durable gateway audit store. |
...
| Gateway administration | Hosted auth/session attribution and role authorization before mutations; provenance review, provider profiles, cron grant issuance, budget control, deployment control, topology control, and marketplace installation ignore browser-supplied attribution when a hosted principal exists. Topology updates reject cross-client workspace binding, and marketplace file writes are preceded by hash-only authority. | User/password verifier material, decision metadata, template content, and workspace-root evidence are omitted from DTOs/audit inputs. | Local roles are not tenancy, SSO, or enterprise identity; failed marketplace provisioning is not an automatic rollback boundary. |
| Deployment/cells | `GatewayDeploymentControl` writes target authorization before app-state mutation and exact-confirmed execution decisions before driver invocation; deployment runs retain the same decision metadata, and hosted callers retain principal attribution. | Browser DTOs expose only safe status, never decision/config metadata. | Docker/WSL are not VM isolation; generated Kubernetes controls are not production certification. |
