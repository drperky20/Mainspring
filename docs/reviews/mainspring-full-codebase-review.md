...
| Bounded history is uneven | Activity, approvals, public traces, detailed usage, and artifact inventory are paginated; the primary snapshot still aggregates numerous unrelated collections. | Long-lived local state can increase snapshot cost. | Preserve compatibility now; add endpoint/read-model pagination for audit and memory as concrete callers migrate. |
...
| Inspect outcomes | Runs, artifacts, tool rows, checkpoints, usage, and trace panels exist. | Detailed usage and artifact inventory load through bounded pages; audit and memory history still arrive through the aggregate. | Public RunLog trace, usage, and artifact pages are bounded; progressively disclose the remaining read models. |
