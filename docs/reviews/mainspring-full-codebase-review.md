...
| --- | --- | --- | --- |
| Broad gateway facade | `src/gateway/LocalGateway.ts` remains a large stable public facade. | Ownership is difficult to discover; snapshot/projection work is mixed with domain operations. | Keep the stable facade; `CompatibilityRunPageReader` now owns the bounded compatibility page/cache path, `ApprovalHistoryPage` owns approval-history merging, and further internal collaborators remain a P2 refactor. |
