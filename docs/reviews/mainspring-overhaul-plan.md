...
Last updated: 2026-07-11. This is an implementation plan, not a claim that its
...
| P1 | Introduce bounded/paginated advanced gateway read models after console callers are incremental. | Complete | `GET /runlog/runs`, `GET /runlog/tool-calls`, `GET /compatibility/runs`, `GET /approval-history`, public-only `GET /runlog/runs/:runId/trace`, plus usage, artifact, audit, and per-workspace memory pages use bounded cursors; canonical tool calls use their own restart-safe RunLog cursor and opaque browser IDs, Activity callers load only their visible surface, and compatibility pages keep a 64-session, per-session-invalidated cache without consulting the broad snapshot revision. |
| P1 | Make per-workspace memory operator-correctable without exposing source records. | Complete | Append-only JSONL `memory.replaced`/`memory.deleted` rows replay safely after partial writes; opaque workspace-scoped correction/deletion routes rescan replacement text, persist hash-only host decisions/audit evidence, and the console confirms both actions in a keyboard-safe dialog. |
| P2 | Consolidate `LocalGateway`/`AppStateStore` behind bounded internal services/repositories. | In progress | Existing domain controls own compatibility pages, activity, approvals, tools, memory, provenance, provider profiles, cron, budgets, deployment, topology, marketplace, and run control. `GatewaySnapshotReader` owns aggregate read/revision and RunLog worker-summary behavior; `GatewayProjectionSynchronizer` owns the callback-driven approval, usage, tool-call, artifact, execution-cell, and terminal-lease catch-up pass; `GatewayRunInputResolver` owns shared session/workspace/client, agent, provider-profile, and runtime-profile binding for gateway/RunLog/cron ingress; `GatewayCronScheduler` owns timer lifecycle, due-row filtering, overlap coalescing, and tick status; `GatewayChatStream` owns public RunLog-to-AI-SDK stream projection and terminal closure; and artifact/usage services restrict projection writes, path-safe reads, normalization, pricing, and budget transitions. Remaining gateway/app-state domains still need tested seams. |
| P2 | Make CI and release validation canonical, cross-platform, and diagnosable. | Complete | One Linux/macOS/Windows `pnpm verify` matrix, one real Chromium `pnpm console:e2e` job with retained failure evidence, pinned pnpm activation, and a release workflow invoking the same `pnpm release:check` used locally. |
| P2 | Simplify first-level console navigation and split style tokens/base/shell/primitives/features. | In progress | Primary navigation is already constrained; `ConsoleSettings` owns settings UI and feature CSS, `ConsoleWorkflowPrimitives` owns shared workflow controls and accessible modal focus behavior, `ConsoleClientWorkspaceFrame` owns the selected-client header, workspace context, tabs, and two-column shell, and `ConsoleClientWorkspaceViews` owns chat, agent editing, automation testing, access preview, and tool configuration. `controlRoom.css` now owns shell/activity/state/responsive rules while `controlRoomWorkspace.css` owns client/setup/chat/automation rules. Phone-width client tabs remain reachable in a two-column grid, and the 390px E2E covers all Activity navigation including canonical Tool Calls, no horizontal overflow, and dialog focus return; the desktop E2E traverses every client view with a reviewed automation screenshot. Remaining token/base and finer feature-style splits remain. |
| P2 | Improve explicit redirect/session/trace UX where RunLog semantics exist. | Complete | `ConsoleLocation` validates shareable screen/activity/run state without sensitive URL material; RunLog trace paging and compatibility event loading are session-bound and abortable; focused tests and Chromium deep-link/reload E2E pass. |
