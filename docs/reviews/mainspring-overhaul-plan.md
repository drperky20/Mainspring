...
| P2 | Simplify first-level console navigation and split style tokens/base/shell/primitives/features. | In progress | Primary navigation is already constrained; `ConsoleSettings` owns settings UI and feature CSS, `ConsoleWorkflowPrimitives` owns shared workflow controls and accessible modal focus behavior, and `ConsoleClientWorkspaceFrame` owns the selected-client header, workspace context, tabs, and two-column shell. Phone-width client tabs remain reachable in a two-column grid, and the 390px E2E covers all Activity navigation including canonical Tool Calls, no horizontal overflow, and dialog focus return. Remaining CSS feature-boundary splits remain. |
