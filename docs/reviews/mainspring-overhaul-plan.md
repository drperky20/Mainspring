...
| P2 | Consolidate `LocalGateway`/`AppStateStore` behind bounded internal services/repositories. | In progress | `CompatibilityRunPageReader` now owns metadata pagination, session cache, and mailbox freshness behind the stable public facade; remaining gateway/app-state domains still need their own tested seams. |
