...
| P2 | Consolidate `LocalGateway`/`AppStateStore` behind bounded internal services/repositories. | In progress | `CompatibilityRunPageReader` owns metadata pagination, session cache, and mailbox freshness; `ApprovalHistoryPage` owns canonical/compatibility approval merging; `RunLogActivityPage` owns canonical activity/trace reads; `RunLogToolCallHistoryPage` owns canonical tool-history projection; `GatewayMemoryControl` owns opaque-ID memory correction/deletion; `GatewayProvenanceReviewControl` owns staged review authority plus replay-safe applies; `GatewayProviderProfileControl` owns provider configuration plus managed-secret authority; `GatewayCronControl` owns hash-only schedule/grant authority; `GatewayBudgetControl` owns budget configuration plus warning/block authority; `GatewayDeploymentControl` owns hash-only pre-mutation target authority plus exact-confirmed, principal-attributed driver execution; `GatewayTopologyControl` owns hash-only client/workspace/agent/session authority, cross-client binding, and deletion guards; and `GatewayMarketplaceControl` owns verified template file installation plus topology provisioning. Remaining gateway/app-state domains still need their own tested seams. |
