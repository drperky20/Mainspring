...
older `createMainspring` SDK/gateway compatibility surface and some compatibility examples. This map
...
The compatibility path is not a second product architecture. It is the migration bridge that keeps existing SDK, gateway, and compatibility examples working while public surfaces move to `RunIntent`, RunLog events, and `RunLogProjection`.
...
| --- | --- | --- | --- | --- | --- |
| `src/runtime/RuntimeEngine.ts` | `src/sdk/Mainspring.ts` constructs it for `createMainspring().start()` | `src/core/RunLogKernel.ts` plus a RunLog-backed SDK host | Compatibility runtime loop | `src/index.test.ts`, `src/gateway/LocalGateway.test.ts` | Keep until remaining compatibility examples and embedders have RunLog replacements or deprecation coverage. |
...
| `src/mailbox/SqliteMailbox.ts` | `SqliteMainspringStorage`, `LocalGateway`, runtime tests | `src/adapters/sqlite/SqliteRunLogStore.ts` | Compatibility event journal and inbound queue | `src/mailbox`, `src/gateway/LocalGateway.test.ts` | Keep until SDK/gateway storage writes RunLog rows directly. |
...
| `src/sdk/Mainspring.ts` session/run creation | Compatibility examples and legacy gateway compatibility | `src/sdk/RunLogMainspring.ts` creates `RunIntent` and projects `RunLogProjection` | Public compatibility host plus RunLog-native SDK host. README, `examples/provider-run`, `examples/tool-approval`, `examples/coding-agent`, `examples/personal-assistant`, `examples/support-agent`, `examples/agency-client-agent`, and `openrouter:e2e` now use the RunLog SDK host. The local gateway dev server also creates a RunLog host. | `src/index.test.ts`, `src/sdk/RunLogMainspring.test.ts`, `scripts/check-agentic-harness.mjs`, examples smoke, `scripts/openrouter-e2e.mjs` | Keep `createMainspring` until compatibility examples are migrated or intentionally retained; use `createRunLogMainspring` for new SDK work. |
...
| `docs/operations.md` OpenRouter E2E runtime lane | Maintainer live-provider verification | `createRunLogMainspring` live-provider E2E | RunLog-native optional verifier with fail-closed missing-key diagnostics | `scripts/openrouter-e2e.mjs`, `scripts/check-optional-verifiers.mjs`, `pnpm openrouter:e2e` | Keep as optional live-provider evidence outside release checks because it needs a real key and network path. |
...
2. Add taint labels beyond scan findings for web/email/file-derived memory and skill mutations.
3. Add checkpoint replay/retry controls only after RunLog replay semantics are code-backed.
