...
older `createMainspring` SDK/gateway compatibility surface. This map
...
| --- | --- | --- | --- | --- | --- |
| `src/runtime/RuntimeEngine.ts` | `src/sdk/Mainspring.ts` constructs it for `createMainspring().start()` | `src/core/RunLogKernel.ts` plus a RunLog-backed SDK host | Compatibility runtime loop | `src/index.test.ts`, `src/gateway/LocalGateway.test.ts` | Keep until legacy embedders and gateway compatibility surfaces have RunLog replacements or deprecation coverage. |
| `src/runner/SessionRuntimeSupervisor.ts` | `RuntimeEngine` polls session mailboxes | `src/core/RunLogScheduler.ts` claim/drain loop | Compatibility poller. The legacy runner now resolves each persisted session workspace independently and places metadata-free sessions in separate subdirectories beneath the configured workspace root. This path separation is not a sandbox. | `src/runner/SessionRuntimeSupervisor.test.ts`, `src/runner/SessionWorkspace.test.ts` | Keep while existing per-session mailboxes are public SDK/gateway storage. |
