...
type CompatibilityRun = ConsoleGatewaySnapshot['runs'][number]

export type CompatibilityRunPageState = {
...
const initialState: CompatibilityRunPageState = {
  runs: [],
  loading: false,
  loaded: false,
}

export function appendCompatibilityRunPage(
  current: CompatibilityRun[],
  next: CompatibilityRun[],
): CompatibilityRun[] {
...
/**
 * Loads compatibility history only when Activity > Runs is open. It keeps the
 * mailbox-backed lane separate from the canonical RunLog activity feed while
 * preserving one merged operator timeline in the shell.
 */
export function useCompatibilityRunPage(input: {
  enabled: boolean
  gatewayClient: LocalGatewayClient
  revision?: string
  limit?: number
}): CompatibilityRunPageState & {
  reload(): Promise<void>
  loadMore(): Promise<void>
} {
  const [state, setState] = useState<CompatibilityRunPageState>(initialState)
  ...
  const loadPage = useCallback(async (options: { append: boolean; cursor?: string }) => {
    ...
    const request = (async () => {
      ...
      try {
        const page = await input.gatewayClient.compatibilityRuns({
        ...
        if (!isCurrent()) return
        setState((current) => ({
          runs: options.append ? appendCompatibilityRunPage(current.runs, page.runs) : page.runs,
